...
class ResNet(SimpleNet):
    def __init__(self, block, num_blocks, num_classes=10, name=None, created_time=None):
        ...
        self.layer5 = self._make_layer(block, 512, num_blocks[2], stride=2)
        self.layer6 = self._make_layer(block, 1024, num_blocks[3], stride=2)
        # self.linear = nn.Linear(256*block.expansion, num_classes)
        self.linear = nn.Linear(1024*block.expansion, num_classes)

        #########################################################################################
        # 这里的模型需要改，在cifar10 下，模型就4层，到256结束，但在FMNIST 下，模型6层到1024结束         #
        #########################################################################################
    ...
    def forward(self, x):
        ...
        #######################################################################################################
        # 这里的模型需要改，在cifar10 下，out = F.avg_pool2d(out, 4)，但在FMNIST 下，out = F.avg_pool2d(out, 2)    #
        #######################################################################################################
...
def ResNet101(name=None, created_time=None):
    return ResNet(Bottleneck, [3,4,23,3],name='{0}_ResNet_101'.format(name), created_time=created_time)

def ResNet152(name=None, created_time=None):
    return ResNet(Bottleneck, [3,8,36,3],name='{0}_ResNet_152'.format(name), created_time=created_time)
